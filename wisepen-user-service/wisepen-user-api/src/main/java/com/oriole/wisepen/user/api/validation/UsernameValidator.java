...
public class UsernameValidator implements ConstraintValidator<Username, String> {
    ...
    private static final Pattern USERNAME_PATTERN = Pattern.compile("^(?=.*[a-zA-Z])[a-zA-Z0-9_]{4,20}$");

    // 11位数字格式（不合法）
    private static final Pattern ELEVEN_DIGIT_PATTERN = Pattern.compile("^\\d{11}$");

    // 3位数字+XH+4位数字格式（不合法）
    private static final Pattern XH_PATTERN = Pattern.compile("^\\d{3}XH\\d{4}$");
